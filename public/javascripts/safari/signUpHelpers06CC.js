...
var helper = {
    ...
    initializeJqueryEvents:  function(){
        ...
        $('#signUpForm').on('submit', function(e) {
            ...
            if (!formValid){
                ...
                if(is_safari){
                    ...
                    console.log('#signUpForm > SUBMIT +++ DOING ALL EVENTS: ');
        ...
        */
        ...
        $('#password').on('focusout change', function(e) {
            console.log('At ' + helper.formatDate(new Date()) + '#signUpForm > change: ', $(this).attr('id'));
            if (e.type === 'change') {
                console.log('At ' + helper.formatDate(new Date()) + 'PASSWORD type CHANGE +++');
                $(this).data('signUpFormActiveElement', $(document.activeElement).attr('id'));
            }
            helper.handleElementEventsOnSubmit('password', true, e.type);
        });

        $('#confirmPassword').on('focusout change', function(e) {
            console.log('At ' + helper.formatDate(new Date()) + '#signUpForm > change: ', $(this).attr('id'));
            helper.handleElementEventsOnSubmit('confirmPassword', true, e.type);
        });
        ...
        $('#signUpForm').get(0).addEventListener('change', function(e){
            console.log('BBOOGER!!: ', $(e.target).attr('id'));
        ...
        /*
        $('#signUpSubmitBtn').on('click', function(e) {
            console.log('At ' + helper.formatDate(new Date()) + ' #signUpSubmitBtn > click!');
            //e.preventDefault();
            //$( "#signUpForm" ).submit();
        });

        $('#displayname').on('focusout', function(e) {
            console.log('At ' + helper.formatDate(new Date()) + ' #displayname > focusout!');
            helper.handleElementEventsOnSubmit('displayname');
        });

        $('#password').on('focusout', function(e) {
            console.log('At ' + helper.formatDate(new Date()) + ' #password > focusout!');
            helper.handleElementEventsOnSubmit('password', true, e.type);
        });
        $('#password').on('change', function(e) {
            console.log('At ' + helper.formatDate(new Date()) + ' #password > change!');
            $('#signUpForm').data('signUpFormActiveElement', $(document.activeElement).attr('id'));
            helper.handleElementEventsOnSubmit('password', true, e.type);
        });

        $('#confirmPassword').on('focusout', function(e) {
            console.log('At ' + helper.formatDate(new Date()) + ' #confirmPassword > focusout!');
            helper.handleElementEventsOnSubmit('confirmPassword', true, e.type);
        });
        $('#confirmPassword').on('change', function(e) {
            console.log('At ' + helper.formatDate(new Date()) + ' #confirmPassword > change!');
            $('#signUpForm').data('signUpFormActiveElement', $(document.activeElement).attr('id'));
            helper.handleElementEventsOnSubmit('confirmPassword', true, e.type);
        });
    ...
    handleElementEventsOnSubmit: function(elementID) {
        console.log('handleElementEventsOnSubmit +++', elementID);

        $('#signUpForm').data('confirmElement', false);
        elementID === 'displayname' ? helper.displaynameOnFocusout(elementID) : null;
        elementID === 'email' ? helper.emailOnFocusoutChange(elementID) : null;
        elementID === 'confirmEmail' ? helper.emailOnFocusoutChange(elementID) : null;
        elementID === 'password' ? helper.passwordOnFocusoutChange(elementID, 'confirmPassword') : null;
        elementID === 'confirmPassword' ? helper.passwordOnFocusoutChange(elementID, 'password') : null;
    ...
    passwordOnFocusoutChange: function(element, comparedElement) {
        console.log('passwordOnFocusoutChange ++');
        var thisElement = $('#'+element);
        var pattern = /^\S{4,}$/;

        if(helper.validateParams('password', 'confirmPassword')){
            $('#'+comparedElement).off('input');
        }
        thisElement.on('input', function(){
            helper.testUserInput(element,pattern);
        });
    ...
    emailOnFocusoutChange: function(element) {
    ...
    selectElementOnFocusoutChange: function(element) {
