// cd documents/node3/oct31tonov1/wedfeb01

/*
.on(eventType[, selector][, data], handler)

.. the statement binds the inline anonymous function as the click event handler for every 'img' element ...
...
// O'REILLY: Web Development with Node & Express' Chapter 11 (Email/Nodemailer example)
// web servers listen for requests, parse those requests, and send responses
/*

http://stackoverflow.com/questions/967096/using-jquery-to-test-if-an-input-has-focus

Three places that we can add validation (page 238):
	1. At the schema level, using Mongoose, before the data is saved
	2. At the application level, before the data is posted to the API
	3. At the client side, before the form is submitted

	We are going to validate the 'postdata = {}'. 
	That will either be in on the server/controllers OR on the API with AJAX submitted data.
...
￼		} else {
	...
	http://www.stackoverflow.com/questions/21196198/email-validation-using-nodejs-express-and-ajax&udata=a33370bac7cf5f261df94b99715040ec
*/
...
app.use(function(req, res, next){
  ...
  if((s.test(req.headers['user-agent'])) && (!c.test(req.headers['user-agent']))){
  	req.session.isSafari = true;
  }else{
  	req.session.isSafari = false;
  }
